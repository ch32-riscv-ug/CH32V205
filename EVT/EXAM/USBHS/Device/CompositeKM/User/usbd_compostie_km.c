...
volatile uint16_t KB_Scan_Result = 0x7800;                                      // Keyboard Keys Current Scan Result
volatile uint16_t KB_Scan_Last_Result = 0x7800;                                 // Keyboard Keys Last Scan Result
...
void TIM3_IRQHandler( void )
{
    if( TIM_GetITStatus( TIM3, TIM_IT_Update ) != RESET )
    {
        ...
        /* Start timing for uploading the key value received from USART4 */
        if( USART_Send_Flag )
        {
            USART_Send_Cnt++;
        }
 ...
 * @fn      USART4_Init
 *
 * @brief   Initialize UART4 to receive keyboard data sent through the
 *          PC serial software.
 *
...
void USART4_Init( uint32_t baudrate )
{
    ...
    GPIO_Init( GPIOA, &GPIO_InitStructure );
    GPIO_PinAFConfig(GPIOA,GPIO_PinSource14,GPIO_AF4);

    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_15;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;
    GPIO_Init( GPIOA, &GPIO_InitStructure );
    GPIO_PinAFConfig(GPIOA,GPIO_PinSource15,GPIO_AF4);
...
void USART4_IRQHandler( void )
{
    ...
    {
        /* Save the key value received from USART4 */
        USART_Recv_Dat = USART_ReceiveData( USART4 ) & 0xFF;
    }
}

/*********************************************************************
 * @fn      USART4_Receive_Handle
 *
 * @brief   This function handles the key value received from USART4.
 *
 * @return  none
 */
void USART4_Receive_Handle( void )
{
    uint8_t status;
    ...
    else if( flag == 1 )
    {
        ...
        {
            memcpy(USBHSD_UEP_TXBUF(DEF_UEP1), KB_Data_Pack, sizeof( KB_Data_Pack ));
            USBHSD->UEP1_TX_LEN = sizeof( KB_Data_Pack );
...
void KB_Sleep_Wakeup_Cfg( void )
{
    ...
    EXTI->INTENR |= EXTI_INTENR_MR11 | EXTI_INTENR_MR12 | EXTI_INTENR_MR13 | EXTI_INTENR_MR14;
}
...
void KB_Scan( void )
{
    ...
    if( ( scan_cnt % 10 ) == 0 )
    {
        ...
        if( scan_result == ( GPIO_ReadInputData( GPIOB ) & 0x7800 ) )
        {
            KB_Scan_Done = 1;
    ...
    else if( ( scan_cnt % 5 ) == 0 )
    {
        ...
        scan_result = ( GPIO_ReadInputData( GPIOB ) & 0x7800 );
    }
}
...
void MS_Scan_Handle( void )
{
    ...
    if( flag )
    {
        ...
        {
            memcpy(USBHSD_UEP_TXBUF(DEF_UEP2), MS_Data_Pack, sizeof( MS_Data_Pack ));
            USBHSD->UEP2_TX_LEN = sizeof( MS_Data_Pack );
