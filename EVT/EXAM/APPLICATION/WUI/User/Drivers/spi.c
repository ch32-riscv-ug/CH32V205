...
void SPI1_Init(void)
{
    ...
    RCC_PB2PeriphClockCmd(RCC_PB2Periph_GPIOA | RCC_PB2Periph_AFIO | RCC_PB2Periph_SPI1, ENABLE);
...
void SPI1_Read_DMA_Init()
{
    ...
    DMA_InitStructure.DMA_Memory0BaseAddr    = (uint32_t)&DummyByte;
...
void SPI1_Read_DMA(uint8_t* data, uint16_t len)
{
    ...
    DMA_ClearFlag(DMA1, DMA_FLAG_TC3);
    DMA_ClearFlag(DMA1, DMA_FLAG_TC3);
