 ...
 * PIOC IO0 needs to connected  a 4.7k pull-up resistor when chip drives DS1820 mode.
 * PIOC IO0 or IO1 can be used to drive WS2812.the Fsys default 160MHz
 * PIOC-RAM:20000000--20000fff(4KB)
 */
...
// long data
        ...
        if ( stat != 0xFF ) {//RGB1W completed
            ...
            if ( stat == 0x80 && total_bytes && timer_to_run ) {//RAM mode for 1~2560 bytes data
                stat = 0xFF;//wait
                if(t1==0){
                    t1 = 1;
                    RGB_RAM = RGBpbuf;
